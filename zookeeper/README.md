# Zookeeper
주키퍼는 기본적으로 분산 시스템을 위한 코디네이터다. 고가용성을 지원하도록 설계 되어 있으며 `zab` 합의 알고리즘을 사용한다.

주키퍼 기본 Java api 를 사용하기도 하지만 `Apache Curator` 라는 고수준 추상화 API 도 있다.
...
### 주키퍼 구성 시 고려사항
- 주키퍼를 배포할 때는 반드시 홀 수로 배포 한다. (쿼럼/합의 참고)
- 규모가 크면 5대 작으면 3대 정도가 적당하다. 서버를 더 추가하면 쓰기 지연이 증가하고 과반(합의) 의 크기가 커지므로 읽기 부하를 수용할 수 있는 것 보다 더 큰 수의 쿼럼을 구성하지 않도록 한다.
